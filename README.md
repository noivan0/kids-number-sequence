...
이 저장소의 공개 README는 게임 소개와 사용 방법에 집중합니다.

상세한 기획, 프리미엄 기능 설계, 마케팅 전략, B2B 자료 등은 **로컬 전용 문서**로 관리하고 있습니다:

- `PREMIUM_STRATEGY_SPEC.md` — 프리미엄 기능 상세 스펙, 공유카드·부모리포트 설계, 전체 전략
- `b2b/` 폴더 — 실제 영업용 제안서 및 워크시트 샘플 (HTML → PDF 변환 추천)
- `MARKETING_CHANNELS.md` — 홍보 채널 리서치 리스트 (별도 작성)

공개 저장소에는 수익화 관련 구체적인 가격·전략 내용은 포함하지 않습니다.
...
## 🏫 B2B 자료 (유치원·어린이집 실제 영업용)

`b2b/` 폴더에 실제로 발송할 수 있는 자료를 준비했습니다:

- **B2B_제안서_숫자순서대로_v1.html** — 1페이지 전문 제안서 (A4 인쇄 최적화)
- **워크시트_샘플_10씩더하고빼기_v1.html** — 실제 수업용 2페이지 활동지 + 교사 가이드

**사용법**: 파일을 Chrome으로 열고 `Ctrl+P` → "PDF로 저장" (여백 최소)

이 자료들로 실제 유치원·어린이집에 사용 요청을 보내실 수 있습니다.  
추가로 필요하신 자료(전체 워크시트 세트, 가정통신문, 계약서 초안 등)가 있으면 바로 만들어 드리겠습니다.